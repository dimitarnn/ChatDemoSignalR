...
function AddToGroup(roomName) {
    ...
    function getNotifications() {
        ...
        $.get('/Notification/GetNotifications', function (data) {
            // hide notification icon if there are no notifications

            console.log(data.count + ' ' + window.UserName);
...
connection.on('ReceiveNotification', function () {
    ...
    connection.on('ReceiveMessage', function (message) {
        ...
        //$.get('/Home/GetUserName', function (data) {
        //    username = data;
        ...
        if (window.UserName != null && message.sender == window.UserName) {
            spanSender.style.color = '#e053b3';
        }
        else {
            //getNotifications(); /// receive notification if the user isn't the one sending the message
        }
    ...
    $('#groupSendButton').on('click', function (event) {
        ...
        $.post('/Chat/SendMessage', data)
            .done(function (response) {
                ...
                connection.invoke('SendNotificationToGroup', roomName).catch(function (err) {
                    return console.error(err.toString());
                });
            })
